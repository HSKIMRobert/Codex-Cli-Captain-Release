...
현재 crates.io release는 `v0.0.28`입니다. crates.io package는 visible WAVE
...
crates.io에 publish된 `v0.0.28` release를 사용하고, 설치 후 plugin 지원까지
...
| 명령 | 용도 |
| --- | --- |
| `cargo install codex-cli-captain --version 0.0.28 --force` | crates.io에서 CCC binary를 설치하거나 업데이트합니다. |
...
범위 참고: `v0.0.28`는 App-native role/team-mode 경로를 유지하면서 WAVE
lifecycle과 ODYSSEY planning proof가 operator에게 보이도록 정리합니다. 검증된
release 범위에는 visible WAVE LAUNCH/ANCHOR, Scout 기반 graph-card
preflight, ODYSSEY lane rationale과 host update_plan progress, global rule
consumption, LSP safety, compact rehydration, role fast-mode 설정,
