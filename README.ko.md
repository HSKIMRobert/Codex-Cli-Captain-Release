...
현재 crates.io release는 `v0.0.24`입니다. crates.io package는 App-native
...
| --- | --- |
| Version | `v0.0.24` |
| crates.io | `codex-cli-captain 0.0.24` publish 완료 |
| Git tag | `v0.0.24` |
| GitHub Release | [`v0.0.24`](https://github.com/HoRi0506/Codex-Cli-Captain/releases/tag/v0.0.24) |
| Release commit | `201cff4` |
| Published install | `cargo install codex-cli-captain --version 0.0.24 --force` 성공 |
| 설치 검증 | `ccc --version` -> `0.0.24` |
...
crates.io에 publish된 `v0.0.24` release를 사용하고, 설치 후 plugin 지원까지
...
범위 참고: `v0.0.24`는 짧은 자연어 `$ccc` 요청을 installed orchestrator 경로에
유지하면서 App-native proof binding을 더 엄격하게 합니다. 검증된 release 범위에는
WAVE activation, Scout/graph-card preflight, ODYSSEY lane contract와 lane queue,
host update_plan mirror/ACK, Tactician/Scout/Raider/Scribe/Ghost/Arbiter/Sentinel/Oracle
역할 표면, App-native lifecycle receipt, docs read-only proof, same-run durable fan-in,
