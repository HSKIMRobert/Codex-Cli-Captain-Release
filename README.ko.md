...
release-bundle fallback installer는 기본적으로 `v0.0.5`에 고정되어 있으며, `v0.0.6` fallback bundle asset이 publish 및 검증될 때까지 유지됩니다. Cargo가 `0.0.6`의 기본 설치 경로입니다. `CCC_VERSION`은 의도적으로 다른 release-bundle fallback을 설치할 때만 설정하세요.
