...
現在の crates.io release は `v0.0.28` です。crates.io package は visible WAVE
...
crates.io に publish 済みの `v0.0.28` release を使い、install 後に
...
| コマンド | 用途 |
| --- | --- |
| `cargo install codex-cli-captain --version 0.0.28 --force` | crates.io から CCC binary をインストールまたは更新します。 |
...
Scope note: `v0.0.28` は App-native role/team-mode path を維持しつつ、
WAVE lifecycle と ODYSSEY planning proof を operator に見える形に整えます。
検証済みの release 範囲には visible WAVE LAUNCH/ANCHOR、Scout-backed
